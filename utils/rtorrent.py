...
class Rtorrent:
    ...
    def get_download_total(self):
        total_bytes = 0
        try:
            total_bytes = self.xmlrpc.get_down_total()
        except Exception:
            pass
    ...
    def get_upload_total(self):
        total_bytes = 0
        try:
            total_bytes = self.xmlrpc.get_up_total()
        except Exception:
            pass
        return misc.bytes_to_string(total_bytes)
    ...
    def get_download_rate(self):
        total_rate = 0
        try:
            total_rate = self.xmlrpc.get_down_rate()
        except Exception:
            pass
    ...
    def get_upload_rate(self):
        total_rate = 0
        try:
            total_rate = self.xmlrpc.get_up_rate()
        except Exception:
            pass
        return "%sps" % misc.bytes_to_string(total_rate).replace('B', 'b')
    ...
    def get_torrent_counts(self):
        ...
        total_torrents = 0
        try:
            torrents = self.xmlrpc.d.multicall("main", "d.get_name=", "d.get_custom1=", "d.get_ratio=",
